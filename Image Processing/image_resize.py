...
def blur(image):
    kernels =[3, 5 , 9, 13] #the more we increase the size of the kernel the blurrier the image gets
    for idx, k in enumerate(kernels):
        image_bl=cv2.blur(image, ksize = (k,k))
        cv2.imshow(str(k), image_bl)
        cv2.waitKey(0)
    return #this works out by averging neighboring pixels
...
for filename in listofOfiles:
    if fnmatch.fnmatch(filename, pattern):
        filename, new_image =resize(filename, width,height)
        cv2.imwrite("new_foler\\" + filename, new_image)
